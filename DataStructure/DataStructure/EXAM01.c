...
int Print_All_Data(void)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		...
		printf("[%d] ", i);
		printf("ID:%d\t", exam[i].id);
		printf("NAME:%s\t", exam[i].name);
		printf("SCORE:%d\n", exam[i].jumsu);
	}
	return i;
}
...
int Print_Data(int no)
{
	...
	printf("ID:%d\t", exam[no].id);
	printf("NAME:%s\t", exam[no].name);
	printf("SCORE:%d\n", exam[no].jumsu);
...
int Search_Complete_Data(SCORE * p)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		if ((exam[i].id == p->id) && strcmp(exam[i].name, p->name) == 0 && (exam[i].jumsu == p->jumsu))
		{
			return i;
		}
		// if(memcmp(p,&exam[i],sizeof(SCORE))==0)
	}
	return -1;
}
...
int Copy_All_Data(SCORE * p, int max)
{
	...
	for (n = 0; n<MAX_ST; n++)
	{
		if (exam[n].id == 0) break;
	}
	max = (max>n) ? n : max; // 더 작은 값을 이용. 유효한 데이터보다 max가 크면 유효 데이터 수를 사용
...
int Insert_Node(SCORE * head, SCORE * d)
{
	...
	// if((head->next == (SCORE *)0x0 || (d->id < head->next->))
	if (Head.next == NULL)
	{
		Head.next = d;
		return 1;
...
int Print_Node(SCORE * head, int id)
{
	SCORE *pt = Search_Id_Node(head, id);

	if (pt == NULL) return -1;

	printf("ID=%d, NAME=%s, SCORE=%d, next = 0x%.8X\n", \
		pt->id, pt->name, pt->jumsu, pt->next);
	return 1;
}
...
int Copy_All_Node(SCORE * head, SCORE * buf)
{
	...
	{
	if(head->next == NULL) break;
	buf[i]=*head->next;				// 링크에 연결된 순서대로 복사.
	buf[i].next=&buf[i+1];			// buf의 다음 주소를 가리키도록 생성.
	head=head->next;
	}
	buf[i-1].next=NULL:					// 마지막 데이터라는 것을 가리키기 위해.
	return i;
	*/
}
...
{
	/*
	if(Count_Data()==MAX_ST) return -1;

	while(head->nid != NULL || head->njumsu != NULL)
	{
	...
	if(head->njumsu->jumsu > d->jumsu)
	{
	d->njumsu=head->njumsu;
	head->njumsu=d;
	}
	if(d->nid != NULL && d->njumsu != NULL) return 1;
	}
	...
	*/
	...
	head = temp; // nid를 지정하고, 처음 위치로 초기화해서 njumsu 처리.

	for (i = 0; i<MAX_ST; i++)
	{
		if ((head->njumsu == NULL) || (d->id < head->njumsu->jumsu))
		{
			d->njumsu = head->njumsu;
			head->njumsu = d;
			return 1;
...
int Print_All_Id_Node(SCORE * head)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		...
		printf("addr = 0x%.8X, ID=%d, NAME=%s, SCORE=%d, nid = 0x%.8X, njumsu = 0x%.8X\n", \
			head->nid, head->nid->id, head->nid->name, head->nid->jumsu, head->nid->nid, head->nid->njumsu);
		head = head->nid;
	}
	return i;
}
...
int Print_All_Jumsu_Node(SCORE * head)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		...
		printf("addr = 0x%.8X, ID=%d, NAME=%s, SCORE=%d, nid = 0x%.8X, njumsu = 0x%.8X\n", \
			head->njumsu, head->njumsu->id, head->njumsu->name, head->njumsu->jumsu, head->njumsu->nid, head->njumsu->njumsu);
		head = head->njumsu;
	}
	return i;
}
...
int Delete_Id_Node(SCORE * head, int id)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		...
		if (head->nid->id == id)
		{
			Delete_One_Jumsu_Node(tmp, head->nid->jumsu, id); // 포인터 head가 변경되기 전에 함수 호출해야 한다.
			head->nid->id = 0;
			head->nid = head->nid->nid;
			return 1;
		}
...
void Delete_One_Jumsu_Node(SCORE * head, int jumsu, int id)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		if ((head->njumsu->id == id) && (head->njumsu->jumsu == jumsu))
		{
			head->njumsu = head->njumsu->njumsu;
			return;
		}
...
int Delete_Jumsu_Node(SCORE * head, int jumsu)
{
	...
	for (;;)
	{
		tmp = Search_Jumsu_Node(tmp, jumsu); // 점수를 가지는 주소를 리턴, 값이 있을 때마다 자동으로 주소값이 갱신된다.
		if (tmp == NULL) return -1;

		Delete_Id_Node(head, tmp->id);
		return 1;
	}
}
...
int Insert_Node(SCORE * head, SCORE * d)
{
	...
	for (i = 0; i<MAX_ST; i++)
	{
		...
		if (head->next->id > d->id)
		{
			...
			if (head->next != NULL) head->next->prev = d; // head->next==null은 다음 개체 없다는 것. 복사할 필요없음. 마지막에 삽입.
			head->next = d;
...
int Delete_Node(SCORE * head, int id)
{
	...
	pt->id = 0;
	if (pt->prev)pt->prev->next = pt->next;	// null값이 next나 prev 값을 가질 수 없다.
	if (pt->next) pt->next->prev = pt->prev;	// 따라서, 대입하기 전, null값을 가지는지 확인.
...
int Pop_Stack(int *p)
{
	...
	(*p) = Stack[Sptr];	// 데이터를 꺼내와서 포인터 p에 전달.
	Sptr++;
	return 1;
}
...
int Print_Stack(void)
{
	...
	return MAX_STACK - Sptr;    // 전체 스택에 담겨있는 데이터 수
}

int Count_Full_Data_Stack(void)
{
	return STACK_EMPTY - Sptr;	// 데이터를 가진 스택의 수
}

int Count_Empty_Data_Stack(void)
{
	return Sptr - STACK_FULL;	// 데이터가 없는 스택의 수
}
...
int In_Queue(int data)
{
	int i, n, cnt = Count_Full_Data_Queue();

	if (Wrptr == Q_FULL)
	{
		if (Rdptr == Q_EMPTY) return -1;

		n = Rdptr - (MAX_Q - Q_FULL);
		...
		Wrptr -= n;
		/*
		for(i=0;i<Wrptr-Rdptr);i++)	// Rdptr앞 비어있는 Queue의 수까지 반복.
		{
		Queue[i]=Queue[Rdptr+i];
		}
		Wrptr-=Rdptr;  // 기존의 Rdptr 위치로 이동
		Rdptr=Q_EMPTY; // Queue의 처음 위치로 이동
		*/
	}
	Queue[Wrptr] = data;
	Wrptr++;
...
int In_Queue(int data)
{
	if (Count_Full_Data_Queue() == MAX_Q) return -1;
...
int Count_Full_Data_Queue(void)
{
	if (Rdptr > Wrptr) return MAX_Q - (Rdptr - Wrptr); // linear에서는 불가능하지만 circular에서는 가능하다
	return Wrptr - Rdptr;
}
...
int Delete_Jumsu_Node(SCORE * head, int jumsu)
{
	...
	for (;;)
	{
		p = Search_Jumsu_Node(temp, jumsu);
...
int Push_Stack(STACK *data)
{
	...
	*ps = *data; ps->next = Sptr; Sptr = ps; return 1;
}
...
int In_Queue(QUEUE * data)
{
	...
	{
		*q = *data;
		q->next = NULL; Wrptr = q; Rdptr = q;
		return 1;
	}
	q->next = NULL; Wrptr->next = q; Wrptr = q;
	return 1;
...
int Out_Queue(QUEUE * p)
{
	...
	if (Rdptr->next == NULL && Rdptr == Wrptr) return -1;
...
{
	SCORE * p; p = calloc(1, sizeof(SCORE));

	if (p == (void *)0x0) return p;  *p = *d;
...
int Insert_Data(SCORE * d)
{
	...
	for (i = key, pass = 0; pass<MAX_ST; pass++)
	{
		if (Hash_table[i].id == -1)
		{
			Hash_table[i] = *d; return i;
		}
		i += STEP;
...
int Insert_Data(SCORE * d)
{
	...
	for (;;)
	{
		// 같은 ID를 검사하는 코드 추가 if(h->next->id == p->id) return -2;
...
int Delete_Data(int id)
{
	...
	for (;;)
	{
		if (h->next == NULL || h->next->id > id) break;
